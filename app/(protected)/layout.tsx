...
export default function ProtectedLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen bg-background text-foreground transition-colors">
      <aside className="w-64 border-r border-sidebar-border bg-sidebar p-6 text-sidebar-foreground shadow-sm">
        <h1 className="mb-8 text-2xl font-bold text-sidebar-primary">FinAssist</h1>

        <nav className="flex flex-col space-y-4 text-sm font-medium">
          <Link href="/chat" className="transition-colors hover:text-sidebar-primary">
            Chat Assistant
          </Link>
          <Link href="/applications" className="transition-colors hover:text-sidebar-primary">
            Applications
          </Link>
          <Link href="/chat" className="transition-colors hover:text-sidebar-primary">
            Documents
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex flex-1 overflow-hidden">
          <section className="flex-1 overflow-y-auto bg-background p-6">{children}</section>
